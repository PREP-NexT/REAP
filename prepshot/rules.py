...
class RuleContainer:
    ...
    def cost_rule(self, model):
        ...
        """
        return model.cost == model.cost_var + model.cost_newtech + model.cost_fix + model.cost_newline + model.carbon_cost + model.unmet_demand_cost + model.trans_slack_cost + model.gen_slack_cost + model.carbon_offset_cost - model.income

    def carbon_cost_rule(self, model):
        """
        Rule for carbon emission cost.

        Args:
            model (pyomo.core.base.PyomoModel.ConcreteModel): Model to be solved.

        Returns:
            pyomo.core.base.PyomoModel.ConcreteModel: Model with carbon cost constraints.
        """

        return model.carbon_cost == sum([self.para['carbon_tax'][z, y] * model.carbon_capacity[y, z] * self.para['var_factor'][y] for y,z in model.year_zone_tuples]) / self.para['weight']
    
    def carbon_offset_cost_rule(self, model):
        """
        Rule for carbon emission offset cost via carbon credits.
        ...
        return model.carbon_offset_cost == sum([self.para['carbon_offset_price'][z, y] * model.carbon_offset[y, z] * self.para['var_factor'][y] for y,z in model.year_zone_tuples]) / self.para['weight']
    
    def carbon_offset_limit_rule(self, model, y, z):
        """
        Rule for carbon emission offset limit via carbon credits.

        Args:
            model (pyomo.core.base.PyomoModel.ConcreteModel): Model to be solved.

        Returns:
            pyomo.core.base.PyomoModel.ConcreteModel: Model carbon offset limit constraints.
        """
        if self.para['carbon_emission_limit'][y] <= 0 and y > self.para['year'][0]:
            pre_y = self.para['year'][self.para['year'].index(y) - 1]
            return model.carbon_offset[y, z] <= model.carbon_offset[pre_y, z]
        return model.carbon_offset[y, z] <= self.para['carbon_offset_limit'][z, y] * (model.carbon_capacity[y, z] + model.carbon_offset[y, z])
        
    
    def unmet_demand_rule(self, model):
        """
        Rule for unmet demand.

        Args:
            model (pyomo.core.base.PyomoModel.ConcreteModel): Model to be solved.

        Returns:
            pyomo.core.base.PyomoModel.ConcreteModel: Model with unmet demand constraints.
        """
        flag = False
        for y in self.para['year']:
            if np.inf == self.para['unmet_demand_cost'][y]:
                # If unmet demand cost is infinite, then unmet demand is not allowed.
                flag = True
                break
        if flag:
            return model.unmet_demand_cost + sum(model.unmet_demand[h, m, y, z] for h, m, y, z in model.hour_month_year_zone_tuples) == 0
        else:
            return model.unmet_demand_cost == sum([self.para['unmet_demand_cost'][y] * model.unmet_demand[h, m, y, z] * self.para['var_factor'][y]  
                                                   for h, m, y, z in model.hour_month_year_zone_tuples]) / self.para['weight']
        
    def trans_penalty_rule(self, model):
        """
        Rule for cost of unmet pr-edefined yearly load rate.

        Args:
            model (pyomo.core.base.PyomoModel.ConcreteModel): Model to be solved.

        Returns:
            pyomo.core.base.PyomoModel.ConcreteModel: Model with unmet predefined yearly load rate cost constraints.
        """
        return model.trans_slack_cost == sum(model.trans_slack[y, z, z1] * self.para['low_load_rate_cost'][z, z1] * self.para['var_factor'][y] for y, z, z1 in model.year_zone_zone_tuples) / self.para['weight']
    
    def gen_penalty_rule(self, model):
        """
        Rule for cost of unmet pr-edefined yearly capacity factor of generation.

        Args:
            model (pyomo.core.base.PyomoModel.ConcreteModel): Model to be solved.

        Returns:
            pyomo.core.base.PyomoModel.ConcreteModel: Model with cost of unmet pr-edefined yearly capacity factor of generation constraints.
        """
        flag = False
        for te in self.para['tech']:
            for z in self.para['zone']:
                if np.inf == self.para['low_gen_cost'][te, z]:
                    # If low gen cost is infinite, then unmet generation is not allowed.
                    flag = True
                    break
        if flag:
            return model.gen_slack_cost + sum(model.gen_slack[y, z, te] for y, z, te in model.year_zone_tech_tuples) == 0
        else:
            return model.gen_slack_cost == sum(model.gen_slack[y, z, te] * self.para['low_gen_cost'][te, z] * self.para['var_factor'][y] for y, z, te in model.year_zone_tech_tuples) / self.para['weight']
    ...
    def emission_calc_by_zone_rule(self, model, y, z):
        ...
        """
        return model.carbon_capacity[y, z] == sum(self.para['emission_factor'][te, y] * model.gen[h, m, y, z, te] * self.para['dt'] for h, m, te in model.hour_month_tech_tuples) - model.carbon_offset[y, z]
    ...
    def power_balance_rule(self, model, h, m, y, z):
        ...
        return demand_z == imp_z - exp_z + gen_z - charge_z + model.unmet_demand[h, m, y, z]
    ...
    def trans_low_bound_rule(self, model, y, z, z1):
        """
        Rule for transmission lower bound.
        Added `low load rate cost` parameter to add penalty fees for failure to meet minimum year load factor of 75%
        total yearly transmitted power (MWh)+ Slack ≥ 0.75 * existing capacity (MW) * Time (hour) and Slack ≥ 0
        Objective =+ Slack * penalty fee
        If transmitted power ≥ 0.75 * existing capacity (MW) * Time (hour), then slack = 0
        Args:
            model (pyomo.core.base.PyomoModel.ConcreteModel): Model to be solved.
            y (int): Year.
            z (str): Zone.
        ...
        Returns:
            pyomo.core.base.PyomoModel.ConcreteModel: Model with transmission lower bound constraints.
        """
        return sum(model.trans_export[h, m, y, z, z1] for h,m in model.hour_month_tuples) + model.trans_slack[y, z, z1] >= model.cap_lines_existing[y, z, z1] * self.para['low_load_rate'][z, z1] * len(model.hour_month_tuples)
    ...
    def gen_low_bound_rule(self, model, y, z, te):
        """
        Rule for generation lower bound.
        Added `low gen cost` parameter to add penalty fees for failure to meet minimum year load factor of 75%
        total yearly generated power (MWh)+ Slack ≥ 0.75 * existing capacity (MW) * Time (hour) and Slack ≥ 0
        Objective =+ Slack * penalty fee
        If generated power ≥ 0.75 * existing capacity (MW) * Time (hour), then slack = 0
        Args:
            model (pyomo.core.base.PyomoModel.ConcreteModel): Model to be solved.
            y (int): Year.
            z (str): Zone.
            te (str): Technology.
        """
        return sum(model.gen[h, m, y, z, te] for h,m in model.hour_month_tuples) + model.gen_slack[y, z, te] >= model.cap_existing[y, z, te] * self.para['low_gen'][te, z] * len(model.hour_month_tuples)
    ...
    def tech_up_bound_rule(self, model, y, z, te):
        ...
        if self.para['technology_upper_bound'][z, te, y] == np.inf:
            return Constraint.Skip
        else:
            return model.cap_existing[y, z, te] <= self.para['technology_upper_bound'][z, te, y]

    def tech_low_bound_rule(self, model, y, z, te):
        """
        Rule for technology lower bound.
        ...
        Returns:
            pyomo.core.base.PyomoModel.ConcreteModel: Model with technology lower bound constraints.
        """
        
        return model.cap_existing[y, z, te] >= self.para['technology_lower_bound'][z, te, y]
    ...
    def new_tech_up_bound_rule(self, model, y, z, te):
        ...
        """
        if self.para['new_technology_upper_bound'][z, te, y] == np.inf:
            return Constraint.Skip
        else:
            return model.cap_newtech[y, z, te] <= self.para['new_technology_upper_bound'][z, te, y]
    ...
    def new_tech_low_bound_rule(self, model, y, z, te):
        ...
        """
        return model.cap_newtech[y, z, te] >= self.para['new_technology_lower_bound'][z, te, y]

    def reserve_margin_rule(self, model, h, m, y, z):
        """
        Rule for reserve margin.
        ...
        Returns:
            pyomo.core.base.PyomoModel.ConcreteModel: Model with reserve margin constraints.
        """
        dispatch_cap = (sum([model.cap_existing[y, z, te] for te in model.dispatchable_tech]) + sum([model.cap_existing[y, z, te] for te in model.hydro_tech]))
        return dispatch_cap - self.para['demand'][z, y, m, h] >= self.para['reserve_margin'][z, y] * self.para['demand'][z, y, m, h]
    ...
    def _cost_offset_breakdown(self, model, y, z):
        return (self.para['carbon_offset_price'][z, y] * model.carbon_offset[y, z] * self.para['var_factor'][y]) / self.para['weight']