...
def define_variables(model, para):
    ...
    model.trans_slack_cost = Var(within=NonNegativeReals, doc='Slack variable for transmission line [dollar]')
    model.gen_slack_cost = Var(within=NonNegativeReals, doc='Slack variable for generation [dollar]')
    model.carbon_cost = Var(within=NonNegativeReals, doc='Carbon dioxide emission cost [dollar]')
    model.unmet_demand_cost = Var(within=NonNegativeReals, doc='Penalty cost Unmet demand [dollar]')
    ...
    model.unmet_demand = Var(model.hour_month_year_zone_tuples, within=NonNegativeReals, doc='Unmet demand in each year, each zone and each time period [MWh]')
    ...
    model.trans_slack = Var(model.year_zone_zone_tuples, within=NonNegativeReals, doc='Slack variable for transmission line in each year and each time period [MWh]')
    model.gen_slack = Var(model.year_zone_tech_tuples, within=NonNegativeReals, doc='Slack variable for generation in each year, each zone and each time period [MWh]')
    model.remaining_technology = Var(model.year_zone_tech_tuples, within=NonNegativeReals, doc='remaining technology [MW]')
    model.carbon_offset = Var(model.year_zone_tuples, within=NonNegativeReals, doc='Carbon dioxide emission offset via carbon credits [tonne]')
    model.carbon_offset_cost = Var(within=NonNegativeReals, doc='Carbon dioxide emission offset cost [dollar]')
...
def define_constraints(model, para):
    ...
    model.trans_low_bound_cons = Constraint(model.year_zone_zone_tuples, rule=rules.trans_low_bound_rule, doc='Minimum yearly load rate of transmission lines')
    ...
    model.gen_low_bound_cons = Constraint(model.year_zone_tech_tuples, rule=rules.gen_low_bound_rule, doc='Minimum yearly capacity factor constraint')
    model.tech_up_bound_cons = Constraint(model.year_zone_tech_tuples, rule=rules.tech_up_bound_rule, doc='technology upper bound')
    model.tech_low_bound_cons = Constraint(model.year_zone_tech_tuples, rule=rules.tech_low_bound_rule, doc='technology lower bound')
    ...
    model.reserve_margin_cons = Constraint(model.hour_month_year_zone_tuples, rule=rules.reserve_margin_rule, doc='Reserve margin')
    ...
    model.carbon_cost_cons = Constraint(rule=rules.carbon_cost_rule, doc='Carbon dioxide emission cost')
    model.carbon_offset_cost_cons = Constraint(rule=rules.carbon_offset_cost_rule, doc='Carbon dioxide emission offset cost')
    model.unmet_demand_cost_cons = Constraint(rule=rules.unmet_demand_rule, doc='Penalty cost Unmet demand')
    model.trans_penalty_cost_cons = Constraint(rule=rules.trans_penalty_rule, doc='Penalty cost for transmission line')
    model.gen_penalty_cost_cons = Constraint(rule=rules.gen_penalty_rule, doc='Penalty cost for generation')
    ...
    model.carbon_offset_limit_cons = Constraint(model.year_zone_tuples, rule=rules.carbon_offset_limit_rule, doc='Carbon dioxide emission offset limit')
    ...
    model.cost_offset_breakdown = Expression(model.year_zone_tuples, rule=rules._cost_offset_breakdown)
