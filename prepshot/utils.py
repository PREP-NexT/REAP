...
def update_output_filename(output_filename, args):
    """Append non-None argparse values to the output filename as ``_key_value``.

    ``input_folder`` is excluded because it is a directory path, not a scenario
    suffix — embedding it would put a ``/`` in the output filename.
    """
    suffix_parts = [
        f'{key}_{value}'
        for key, value in vars(args).items()
        if value is not None and key != 'input_folder'
    ]
    if suffix_parts:
        output_filename = output_filename + '_' + '_'.join(suffix_parts)
...
def extract_results_non_hydro(model):
    ...
    cost_carbon_values = model.carbon_cost.extract_values()[None]
    cost_newtech_values = model.cost_newtech.extract_values()[None]
    cost_newline_values = model.cost_newline.extract_values()[None]
    cost_unmet_values = model.unmet_demand_cost.extract_values()[None]
    cost_trans_values = model.trans_slack_cost.extract_values()[None]
    cost_gen_values = model.gen_slack_cost.extract_values()[None]
    ...
    carbon_offset_cost_values = model.carbon_offset_cost.extract_values()[None]
    cost_values = model.cost.extract_values()[None]
    carbon_offset_values = model.carbon_offset.extract_values()
    cost_offset_breakdown_values = model.cost_offset_breakdown.extract_values()
    ...
    gen_by_year_v = gen_v.sum(dim=['month', 'hour'])
    ...
    carbon_offset_v = create_data_array(
        [[carbon_offset_values[y,z] for y in year] for z in zone],
        ['zone', 'year'],
        {'zone': zone, 'year': year}, 'Ton')
    
    cost_offset_breakdown_v = create_data_array(
        [[cost_offset_breakdown_values[y,z]() for y in year] for z in zone],
        ['zone', 'year'],
        {'zone': zone, 'year': year}, 'dollar')
    
    # Calculate total cost and income and create DataArray for each cost component.
    cost_v = xr.DataArray(data=cost_values)
    ...
    cost_carbon_v = xr.DataArray(data=cost_carbon_values)
    cost_unmet_v = xr.DataArray(data=cost_unmet_values)
    cost_trans_v = xr.DataArray(data=cost_trans_values)
    cost_gen_v = xr.DataArray(data=cost_gen_values)
    cost_offset_v = xr.DataArray(data=carbon_offset_cost_values)
    ...
    ds = xr.Dataset(data_vars={'trans_import': trans_import_v,
                               ...
                               'gen_breakdown': gen_v,
                               'gen_by_year':gen_by_year_v,
                               'carbon': carbon_v,
                               'install': install_v,
                               'carbon': carbon_v,
                               'cost': cost_v,
                               'cost_offset': cost_offset_v,
                               'cost_offset_breakdown': cost_offset_breakdown_v,
                               'cost_var': cost_var_v,
                               ...
                               'tax_collection': cost_carbon_v,
                               'cost_unmet': cost_unmet_v,
                               'cost_trans': cost_trans_v,
                               'cost_gen': cost_gen_v,
                               'carbon_offset':carbon_offset_v,
                               'income': income_v})
    return ds
